...
<script type="text/javascript">
		...
		$("#saveRemarkBtn").click(function(){
			...
			$.ajax({
				...
				success: function(data){
					/*
					data:["success":true/false,"remark",{}]
					 */
					if(data.success){
						$("#remark").val("");
						// 添加成功，在textarea上方新增一个div
						var html = '';
		...
		$("#updateRemarkBtn").click(function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.success){
						// 修改成功
						// 更新div，相应的信息，需要更新的内容由 noteContent/editTime/editBy
						$("#e"+id).html(data.remark.noteContent);
						var str = data.remark.editTime + " 由 " + data.remark.editBy;
						$("#s"+id).html(str);
	...
	function showRemarkList(){
		$.ajax({
			...
			success: function(data){
				/*
				data:[{activityRemark1},{activityRemark2}]
				 */
				/*
				...
				$.each(data,function(index,element){
					...
					html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
					html += '<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+element.id+'\',\''+element.noteContent+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>';
					html += '&nbsp;&nbsp;&nbsp;&nbsp;';
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.success){
					// 删除成功
					// showRemarkList();
					// 这种做法不行，记录使用的是before方法，每次删除之后，页面上都会保留原有的数据(追加在原有的数据上)
